...
/**
 * hvigor 插件：在构建前自动生成 BuildConfig.ets
 */
function generateConfigPlugin() {
  return {
    pluginId: 'generate-config-plugin',
    apply(pluginContext) {
      // 在 PreBuild 阶段执行
      pluginContext.registerTask({
        ...
        run: (taskContext) => {
          ...
          const projectRoot = taskContext.getProjectRootPath();
...
export class BuildConfig {
  ...
  public static getConfigInfo(): Record<string, Object> {
    ...
    return configInfo;
      ...
      // 将任务添加到 PreBuild 阶段
      pluginContext.lifecycle.beforeTaskExecute('PreBuild', () => {
        pluginContext.executeTask('generateBuildConfig');
      });
    }
  };
}
